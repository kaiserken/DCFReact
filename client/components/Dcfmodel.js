...
var Dcfmodel  = React.createClass({
  ...
  changeCashFlow: function(event){
    ...
    //cashflow = new Intl.NumberFormat({ style: 'currency', currency: 'USD' }).format(cashflow);
  ...
  computeValue: function(event){
    ...
    companysValue = new Intl.NumberFormat({ style: 'currency', currency: 'USD' }).format(companysValue);
  ...
  console.log(this.state)
    return (
      ...
      <form  className = "cashflows" onSubmit ={this.computeValue} > In the Boxes below - Input your projected cashflow for each of the next 5 years. A good approximation of cash flow is EBITDA minus average annual capital expenditures. EBITDA is earnings before interest, taxes, depreciation and amortization.
        <br/>
        <br/>
        <input id = 'cashFlowYear1' type  = "number"  defaultValue = "" placeholder = "Year 1 cashflow" onChange = {this.changeCashFlow}/>
        <input id  ='cashFlowYear2' type  = "number"  defaultValue = "" placeholder = "Year 2 cashflow" onChange = {this.changeCashFlow}/>
        <input id  ='cashFlowYear3' type  = "number"  defaultValue = "" placeholder = "Year 3 cashflow" onChange = {this.changeCashFlow}/>
        <input id  ='cashFlowYear4' type  = "number" defaultValue = "" placeholder = "Year 4 cashflow" onChange = {this.changeCashFlow}/>
        <input id = 'cashFlowYear5' type  = "number"  defaultValue = "" placeholder = "Year 5 cashflow" onChange = {this.changeCashFlow}/>
        <br/>
        <br/>
        <p>Enter the Discount Rate as a decimal</p>
        <input id  = 'discountRate' type  = "text"  defaultValue = "" placeholder = "Discount Rate" onChange = {this.changeCashFlow}/>
        <br/>
        <p>Enter the Long Term Growth Rate of Cash Flows after year 5 as a decimal</p>
